 <?php
	...
	class Laporan extends CI_Controller
	{
		...
		public function export_excel($dari_tgl, $sampai_tgl, $status_pengaduan, $id_kategori = 'semua', $id_subkategori = 'semua')
		{
			...
			// Ambil nama kategori (jika bukan 'semua')
			...
			// Tampilkan tabel
			echo '<table border="1">';
			echo '<tr><th colspan="11" style="text-align:center; font-size:16pt;">Laporan Kendala Perangkat IT Dari ' . date('d-m-Y', strtotime($dari_tgl)) . ' Sampai ' . date('d-m-Y', strtotime($sampai_tgl)) . ' | Kategori: ' . $nama_kategori . '</th></tr>';
		...
		<th>Status Pengaduan</th>
			...
			foreach ($pengaduan as $p) {
				...
				echo '<td>' . htmlspecialchars(date('d-m-Y H:i', strtotime($p['tgl_pengaduan']))) . '</td>';
				...
				echo '<td>' . htmlspecialchars($p['status_pengaduan'] ?? '-') . '</td>';
				echo '</tr>';
			}
